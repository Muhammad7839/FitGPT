...
The image is sent to Groq's vision endpoint; only clothing items are kept and
PII (card tails, names, addresses, phone numbers, emails) is explicitly barred
from the model's output. Never raises — returns an empty result on any failure.
"""
...
_PROMPT = (
    ...
    "- Top: shirts, tees, blouses, sweaters, hoodies, polos, tank tops\n"
    "- Bottom: jeans, pants, shorts, skirts, trousers, leggings\n"
    "- Outerwear: coats, jackets, blazers, parkas, vests\n"
    "- Shoes: sneakers, boots, loafers, sandals, heels\n"
    "- Accessory: hats, scarves, belts, bags, jewelry, socks, ties\n\n"
    ...
    "(Black, Navy, White). If no color is visible, use an empty string — do not guess.\n\n"
...
def _get_vision_client():
    ...
    try:
        ...
        logger.error("Failed to init Groq vision client: %s", exc)
        return None
...
def extract_clothing_items(image_bytes: bytes, content_type: str) -> ReceiptExtractionResult:
    """Extract clothing line items from a receipt image. Never raises."""
    if not image_bytes:
