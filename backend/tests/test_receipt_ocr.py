"""Tests for the /receipts/ocr endpoint and the receipt_ocr module."""
...
def _png_bytes() -> bytes:
    # A minimal valid 1x1 PNG signature + IHDR + IDAT + IEND (89 bytes).
    return (
        b"\x89PNG\r\n\x1a\n"
...
def test_receipt_ocr_happy_path_extracts_items(client, monkeypatch):
    ...
    monkeypatch.setattr(
        receipt_ocr,
        "_get_vision_client",
        lambda: _FakeClient(content=json.dumps(payload)),
    )
...
def test_receipt_ocr_malformed_response_returns_warning(client, monkeypatch):
    ...
    monkeypatch.setattr(
        receipt_ocr,
        "_get_vision_client",
        lambda: _FakeClient(content="not-json-at-all"),
    )
    response = client.post("/receipts/ocr", headers=auth, files=_upload_fields())
    assert response.status_code == 200
    body = response.json()
