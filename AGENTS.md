...
Recurring `AGENTS.md` maintenance automations are an exception to the normal product-task routing. Treat them as repository documentation maintenance grounded in recent conversation evidence, automation memory, and the current `AGENTS.md`; do not load unrelated product-engineering skills unless the user explicitly asks for a broader repo investigation.
...
If you log a material shared-skill run yourself, save the path printed by the initial `started` invocation and reuse that exact file with `--log-file` for the closing `success` or `failure` update. Do not emit a standalone `started` entry unless you can also close it, because the current logger creates a new file when `--log-file` is omitted and that leaves false stale-run signals in later maintenance sweeps.
