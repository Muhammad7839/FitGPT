...
When reviewing shared skills, treat `$CODEX_HOME/skill-logs/` as the current execution log location and check for both `Current status: failure` and `Current status: started` entries before concluding a workflow is healthy. If the relevant skill files live outside writable roots for the current automation run, do not force broader changes; record the concrete evidence, the blocked file paths, and the exact recommended patch in memory so the next writable run can apply it directly.
